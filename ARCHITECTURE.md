...
| 2026-07-06 | 1.3 | Epic 3 (CF Slice 1+2: `/produk` + `/produk/[slug]`) dan Epic 3B (admin panel `/admin/products` + edit form + upload foto/PDF) COMPLETE, merged ke `main`. Pola baru: Server Component admin fetch produk langsung via `lib/supabase/server.ts` (RLS "authenticated can read all"), BUKAN lewat `apiFetch` — `apiFetch` butuh browser session jadi hanya valid dipanggil dari Client Component. Whitelist edit field di-enforce backend via Pydantic `extra='forbid'`. Upload photo/PDF: filename `{code-slug}-{timestamp}.{ext}`, cleanup file lama best-effort (tidak rollback kalau delete gagal). Cache invalidation via Server Action `revalidateProductRoutes` (termasuk `revalidatePath('/')` — Beranda `ProductsPreview` ternyata render data produk juga, bukan cuma `/produk`). | Epic 3 + 3B selesai — customer-facing katalog + admin CRUD penuh | Claude Code |
> ```
